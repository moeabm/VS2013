...
void Bot_Think( CSDKPlayer *pBot );
...
CBasePlayer *BotPutInServer( bool bFrozen, int iTeam )
{
	...
	pPlayer->ChangeTeam( SDK_TEAM_BLUE );
	pPlayer->HandleCommand_JoinClass( 1 );
...
// Output : 	virtual void
//-----------------------------------------------------------------------------
static void RunPlayerMove( CSDKPlayer *fakeclient, const QAngle& viewangles, float forwardmove, float sidemove, float upmove, unsigned short buttons, byte impulse, float frametime )
{
	if ( !fakeclient )
		return;
